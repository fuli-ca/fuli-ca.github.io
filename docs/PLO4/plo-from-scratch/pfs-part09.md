...
另一种击中并输掉的情况是，在翻牌有获胜的补牌，但在我们击中后，我们的对手可以在河牌击中更好的听牌。例如，如果你在非对子翻牌（例如 A♥J♥9♠8♣ 在翻牌 K♥7♥2♠）上有同花听牌，而对手有顶暗三条，则你有 8 张干净的补牌（除 2♥ 外的所有红心）。但如果你在转牌击中同花，对手有 10 张牌面配对的补牌，可以击中葫芦或四条。因此，你在翻牌的干净补牌实际上少于 8 张。我们将学习使用简单的数学 + 关于对手手牌的假设，将补牌减少为干净补牌。
...
当你翻牌击中对子时，例如 K♦Q♠J♦9♠ 在翻牌 K♥8♠4♦ 时，你有两对 / 三条的听牌。在高 SPR 场景中，这是一种弱听牌（例如跟注和加注底池），但在低 SPR 场景中（例如 3-bet 和 4-bet 底池）可能是一种强听牌。
...
CO（$10）加注到 $0.35，你（$10）持有 Q♦J♦T♠9♥ 在 BTN 位置 3-bet 到 $1.20，盲注弃牌，CO 4-bet 到 $3.75，你跟注。你假设 CO 持有 A-A-x-x，因此你的计划是全押所有你有足够权益对抗 A-A-x-x 的翻牌。
...
BTN（$10）率先溜入，SB（$10）溜入，你（$10）持有 K♠J♦9♠2♣ 在 BB 位置过牌。
...
你（$10）持有 K♦Q♣J♣9♦ 在 BTN 加注到 $0.35，SB（$10）3-bet 到 $1.15，你跟注。SB 是众所周知的紧凶玩家，其 3-bet 范围主要由 A-A-x-x、好的百老汇牌和一些优质投机牌组成。
...
SB（$8.85）下注 $2.40，我们（$8.85）加注全押，SB 跟注。

**Turn：**T♠9♣4♥K♠（$20.10）

**River：**T♠9♣4♥K♠7♥（$20.10）

SB 持有 A♦K♥Q♥J♦ 的牌型与我们相同，但没有对子。翻牌时我们的权益为 66%，如下所示：

![pics/9-3](pics/9-3.jpg)

在 PLO 中，翻牌 66% 的优势是巨大的。但如果我们有相同的包牌但没有一对（例如 K♦Q♣J♣7♦），对手将大幅领先。他现在拥有与我们相同的包牌，并且拥有翻牌 A 高牌的最佳手牌：
...
我们还可以利用后门同花听牌获得有价值的额外权益，例如 A♠K♥J♠T♥ 在 Q♥9♠2♣ 翻牌上，我们有 13 张补牌加上 2 张后门同花听牌。我们通常将后门同花听牌算作 1 张补牌，但当我们有很多对手和听牌较低时，我们可以稍微减少一点（例如减少到 1/2 张补牌）。
...
BTN（$10）加注到 $0.35，你（$10）持有 A♥A♣8♥7♣ 在 SB 位置 3-bet 到 $1.15，BTN 跟注。BTN 是一个稳健的紧凶玩家。
...
在 PLO 中，有同花听牌或没有同花听牌就像是两个不同的世界。在两张同色翻牌中没有同花听牌时，我们经常被迫过牌并放弃或用我们的边缘牌下注 - 弃牌。SPR 越高，当其他人完全有可能拥有同花听牌时，越难从没有同花听牌的牌中获利。但是有了同花听牌，尤其是坚果同花听牌，我们可以下注更多，而 SPR 较低时，我们的决定通常会变得自动（例如，我们很高兴地用一个超对 + 坚果同花听牌在 3-bet 底池中用 100 BB 筹码全押我们的筹码）。
...
CO（$10）溜入，BTN（$10）溜入，SB（$10）溜入，你（$10）持有 A♥8♠7♥4♣ 在 BB 位置过牌。
...
CO（$10）溜入，你（$10）持有 A♣9♠8♠7♣ 在 BTN 加注到 $0.45，SB（$10）跟注，BB（$10）跟注，CO 跟注。
...
CO（$10）加注到 $0.25，你（$10）持有 A♦Q♠J♠T♦ 在 BTN 位置的 3-bet 到 $1.20，盲注弃牌，CO 跟注。CO 看起来紧而直接。
...
你（$10）持有 Q♦J♦T♠6♠ 在 BTN 位置加注到 $0.35，SB（$10）3-bet 到 $1.15，BB 弃牌，你跟注。SB 在不利位置 3-bet 范围很紧，并且你假设 A-A-x-x 占其范围的很大一部分。

**Flop：**9♦8♣4♦（$2.40）

SB（$8.85）下注 $2.40，你的计划是什么？

自动全押。你有强大的 13 补牌坚果包牌加上第三坚果同花听牌，对手很难将你击败。如果他有随机 A-A-x-x，你是遥遥领先：
...
UTG（$10）溜入，CO（$10）溜入，你（$10）持有 A♠T♠9♦4♣ 在 BTN 位置溜入，SB（$10）溜入，BB（$10）过牌。
...
SB 持有 Q♣J♠T♥9♣ 获胜，乐观先生 A♠T♠9♦4♣ 被反超，他现在认为被（ 爆冷输掉）。
...
因此，在这种情况下，我们在转牌有坚果牌，但我们仍然无法通过全押加注赚到一分钱，即使初始转牌底池中有很多死钱。哪里出了问题？
...
然后 SB 在转牌击中坚果顺子，他得到了一个翻牌跟注者，后者所做的正是 SB 所希望的，即用裸的坚果加注全押。请注意，对于 SB 来说，被加注全押比在转牌赢得底池更好。弃掉裸坚果的 EV = 0，而被加注全押给我们 EV = -$1.68。
...
面对这种类型的对手，我们可以得出结论，如果他在河牌危险牌上全押，我们就落后了，我们可以节省一大笔赌注。如果他河牌面对危险牌（例如，一张公共牌配对牌）过牌，我们将有机会通过将我们的顺子变成诈唬并希望 SB 弃掉相同的顺子来赢得整个底池。
...
BTN（$10）加注到 $0.35，你（$10）持有 A♦A♣9♦8♠ 3-bet 到 $1.15，BTN 跟注。

**Flop：**J♥6♦5♣（$2.40）

你后手还有 $8.85。你打算怎么做？

这是一个明显的下注并争取全押的场合。翻牌非常干燥，因此除非 BTN 的范围中有许多神奇的 J-J-x-x / 5-5-x-x / 4-4-x-x / J-5-x-x / J-4-x-x / 5-4-x-x 组合（不太可能），否则你几乎总是领先。如果你落后，你有 6 张坚果补牌听卡顺或顶三条，还有一张后门同花听牌（1 张补牌）。
...
上面的内嵌包牌的所有 9 个补牌（3 张 8、3 张 7、3 张 6）都是坚果牌，所以即使它不是强包牌，也是坚果听牌。在单挑超低 SPR 的情况下，内嵌包牌可能就是我们所需要的。如果它还带有一对或其他额外的权益，那么在翻牌时以中等 SPR 全押就足够了（例如在单挑 3-bet 底池中）。
...
当牌面上有连张（例如 9♠8♦2♣）、1 张缺口连张（例如 J♣9♠4♥）或 2 张缺口连张（例如 A♣9♠6♥）时，就会产生这些情况，而且我们有一些以某种方式包裹牌面的连牌（因此称为 “包牌” ）。
...
这些信息很容易记住，因此是 “唾手可得的果实”。了解这些补牌，以便自动计算补牌数。例如，如果你持有 9♣8♣7♠6♠ 在 A♣5♦4♠ 翻牌，你的思维过程应该是 “叮！我有一个 3 - 0 包牌包裹连张，有 13 张补牌和 13 张坚果补牌”。我可以向你保证，大多数低限额 PLO 玩家对这些事情的了解程度不够。当然，这就是为什么他们经常过度玩非坚果牌，结果却在大底池中惨败。
...
你（$10）持有 9♣8♠7♠6♣ 在 MP 加注到 $0.35，CO（$10）跟注，BTN（$10）跟注，SB（$10）跟注，BB（$10）跟注。
...
CO（$10）跟注，BTN（$10）跟注，SB（$10）跟注，你（$10）持有 A♦K♠Q♦J♠ 在 BB 位置加注到 $0.50，CO 跟注，BTN 跟注，SB 跟注。
...
如果我们在翻牌全押，与坚果牌对决，那么我们将以 45% 的权益全押，底池价值为 $2 + 2 x $9.5 = $21，而我们在翻牌的投资为 $9.50。那么全押的 EV 为：
