...
坚持使用优质手牌可以降低在大底池对抗中处于劣势的可能性，如果我们经常用非坚果手牌与许多对手大打出手，会有很大的风险。因此，Hwang 对起手牌结构非常挑剔。例如，他避免打顶部有缺口的连牌（如 J♦9♠8♠6♥），因为这些手牌会形成许多非坚果顺子。
...
你有一手非优质但可玩的牌（例如 A♥K♠Q♣3♥ 来自 “投机牌” 起手牌类别），在一个或多个溜入者后面。你认为仅从牌面来看，这手牌足够强，可以打，但不一定好到可以加注（而且你不想被 3-bet）。但你还认为加注会增加你的偷牌权益，并使你能够在没有摊牌的情况下赢得更多底池。那么，你是否应该加注以对你后面的玩家施加压力，并增加你在翻牌后偷取底池的机会，即使这手牌不是优质牌？
...
**手牌 1：**T♠9♥8♠5♥（包括在我们的核心策略里）

**手牌 2：**Q♠9♥5♠3♥（不包括在我们的核心策略里）
...
因此，我们可以在溜入者后面加注 T♠9♥8♠5♥ 和类似的投机牌。当加注不会使对手减少时，加注还有其他好处，而且这种手牌通常在大底池中发挥良好，无论加注后发生什么。请注意，我们并不特别担心被 3-bet，因为我们的手牌在 3-bet 或 4-bet 底池中对抗 A-A-x-x 时表现良好（如第 3 部分所示）。

现在，将手牌 2 与手牌 1 进行比较。Q♠9♥5♠3♥ 这是一手非常弱的手牌，看起来比实际情况好得多（因为有两套同花）。这手牌非常不协调，永远不会击中顺子包牌。我们所能期望的最好结果是两头顺子听牌和没有再听牌的顺子。当我们翻牌击中同花 / 同花听牌几乎永远不会是坚果（手牌 1 也是如此，但手牌 1 有很大的顺子潜力，而同花潜力仅仅是后备）。

所以，用这手垃圾牌跟注加注，并不能让我们在很多翻牌圈盈利全押，恰恰相反。当我们击中翻牌时，大多数情况下，我们的牌很弱，并非坚果牌，在多人底池中无法通过下注获得价值。此外，需要注意的是，不协调的牌型从翻牌圈拿免费牌的机会中获益较少（当一手不协调的牌型与翻牌圈配合不佳时，转牌也不太可能带来多大帮助）。因此，隔离加注几乎完全基于偷盲权益，但由于底池中已经有一个或多个溜入玩家，我们的偷盲权益也微乎其微。
...
UTG（$10）溜入，你（$10）在 CO 持有 K♥K♠9♦3♠。你后面的玩家未知。你的计划是什么？
...
UTG（$10）溜入，MP（$10）溜入，你（$10）持有 K♥K♠9♦3♠ 在 BTN 位置。盲注未知。你的计划是什么？
...
请注意，对于像这样的 K♥K♠9♦3♠ 非此即彼的牌，有时在翻牌时获得免费牌的优势几乎不存在。如果我们在翻牌时没有好牌（例如顶暗三条），那么转牌不太可能改善我们的牌力。因此，翻牌前加注的优势消失了，与跟着溜入相比，在这种情况下加注不会给我们带来太多额外的好处。
...
UTG（$10）溜入，你（$10）持有 A♣K♥Q♣6♠ 在 BTN 位置。盲注未知。你的计划是什么？
...
UTG（$10）溜入，MP（$10）溜入，CO（$10）溜入，你（10）持有 A♥T♣7♣4♥ 在 BTN 位置。盲注未知。你的计划是什么？
...
UTG（$10）溜入，MP（$10）溜入，你（$10）持有 J♥9♠8♥6♠ 在 BTN 位置。盲注未知。你的计划是什么？

在这里，我们可以用双同花的连牌加注以隔离，即使它有点粗糙并且顶部有缺口。如前所述，用这种类型的手加注有几个优点：

- 我们用一手非坚果牌来减少对手。
- 当我们很好地击中翻牌时，我们让自己以低 SPR 获利地全押。
...
UTG（$10）溜入，你持有 K♠8♥4♠2♥ 在 BTN 位置。盲注未知。你的计划是什么？
...
CO（$10）加注到 $0.35，你持有 Q♦Q♥J♦T♥ 在 BTN 位置。CO 是一个松散的加注者，VPIP = 48 和 PFR = 29。SB（$10）是未知，而 BB（$10）是一个扎实的紧凶玩家。你的计划是什么？
...
UTG（$10）加注到 $0.35，CO（$10）跟注，BTN（$10）跟注，SB（$10）跟注，你（$10）持有 Q♦Q♥J♦T♥ 在 BB 位置。UTG 是 26/17 的紧凶玩家，其余玩家是未知玩家和松散被动玩家的混合体。你的计划是什么？
...
BTN（$10）加注到 $0.35，SB 弃牌，你（$10）持有 A♠K♥J♥T♠ 在 BB 位置。BTN 未知。你的计划是什么？
...
CO（$10）加注到 $0.35，BTN（$10）跟注，SB（$10）跟注，你持有 A♣A♦J♣T♥ 在 BB 位置。所有对手都是未知的。你的计划是什么？

标准的使用优质牌 A-A-x-x 价值 3-bet。此场景类似于示例 4.4.2（位置不佳，在加注的多人底池中持有优质对子），但这里我们在翻牌前拥有坚果牌，并且我们面对对手中拥有巨大的权益优势并且我们的牌会击中很多翻牌，足以翻牌后继续。此外，我们欢迎 4-bet，因为这将允许我们 5-bet 全押并实现我们所有的翻牌前权益。
...
UTG（$10）加注到 $0.35，MP（$10）跟注，BTN（$10）跟注，你持有 A♣A♥8♣2♥ 在 BB 位置。对手是混合着紧 - 凶和松 - 被动玩家的。你的计划是什么？
...
这使得 A♣A♥8♣2♥ 价值 3-bet 不太合适，因为当我们寻找翻牌来获利全押时，缺乏协调会给我们带来问题（记住：当你根据价值建立一个大的翻牌前底池时，拥有一手经常击中翻牌并足以继续的牌很重要）。

因此，即使拥有两套坚果同花使这手牌成为一手好的 A-A-x-x，缺乏协调也足以让它无法进入优质类别。因此，在这种情况下，最好避免 3-bet，因为在不利位置面对 3 个对手。

请注意，如果你在 BTN 与一个加注者单挑，这将是一个标准的 3-bet，以获得价值 / 隔离。在这种情况下，我们将放弃严格的价值 3-bet 核心策略，而更多地转向投机性 3-bet（尽管 A♣A♥8♣2♥ 这是一手接近优质牌）。
...
UTG（$10）加注到 $0.35，MP（$10）跟注，你（$10）持有 A♠A♥8♣3♠ 在 BTN 位置。所有对手都是未知的。你的计划是什么？
...
UTG（$10）加注到 $0.35，MP（$10）跟注，你（$10）持有 A♠K♥K♠J♥ 在 BTN 位置。UTG 是一个超紧加注者 VPIP = 15 和 PFR = 2.1。你已经看到他在翻牌前加注后两次进入摊牌，而且两次他都是 A-A-x-x。其他玩家未知。你的计划是什么？
...
UTG（$10）加注到 $0.35，MP（$10）跟注，你（$10）持有 A♠K♥K♠J♥ 在 BTN 位置。UTG 是松散激进的 VPIP = 42 和 PFR = 28。MP 是松散被动的。你的计划是什么？
...
- 翻牌后如果他的 4-bet 被跟注，他将全押 A-A-x-x。
- 其余牌他在翻牌上玩 “不中就弃牌” 打法。
...
CO（$10）加注到 $0.35，你持有 T♠9♥8♠6♥ 在 BTN 位置。CO 未知，两个盲注都是紧手的激进玩家，在不利位置玩得很紧。你的计划是什么？
...
UTG（$10）加注到 $0.35，MP（$10）跟注，你持有 A♣9♠8♣7♥ 在 BTN 位置。UTG 是一个扎实的紧凶玩家，只在 UTG 加注优质牌。MP 也是一个紧凶玩家，SB 未知，BB 是松散被动的。你的计划是什么？
...
CO（$10）加注到 $0.35，你持有 Q♠9♥6♠5♣ 在 BTN 位置。你的计划是什么？
...
我们这里暂不做跟注 / 弃牌的决定，但我们会把这个例子带到第 5 部分，在那里找到答案。我们将使用 Q♠9♥6♠5♣ 对抗 A-A-x-x 的翻牌权益分布来决定跟注 4-bet 是否有利可图，然后在翻牌圈我们有足够的权益对抗 A-A-x-x 时全押。
...
1. 我们用 A-A-x-x  5-bet 全押。
2. 我们用协调性足够好的牌跟注 4-bet，然后 “挑选” 翻牌。

第 1 部分很明显，第 2 部分则比较模糊。但主要思想是，对于许多投机性强的牌来说，跟注翻牌前的 4-bet 是有利可图的，计划在我们有足够权益（与翻牌的底池赔率相比）时跟注翻牌的 c-bet 全押。为了使这一策略奏效，我们希望 4-bet 的对手几乎总是持有 A-A-x-x，因为这样可以很容易地准确估计我们在翻牌的权益。

从第 3 部分中，我们知道，像 K♠K♥7♦2♣ 这种非常不协调的牌在 3-bet 和 4-bet 底池中会遇到很大的问题，原因是它只有很少的翻牌有很好的权益。干燥对子（没有帮助的边牌）在对抗 A-A-x-x 时，翻牌权益很高的范围很窄（当我们击中暗三条、明三条或两对时），但除了这些神奇的翻牌之外，没有太多的权益可寻。

因此，当我们用这种牌参与 4-bet 底池时，我们几乎总是不得不对翻牌 c-bet 弃牌。这意味着我们翻牌前的投资没有得到很好的回报，我们只是通过跟注 4-bet 捐献筹码。

下面的翻牌权益分布图说明了 K♠K♥7♦2♣ 对抗 A-A-x-x 在大底池中的问题。我们的大部分翻牌权益都 “挤在一起” 在少数翻牌上（翻牌前 15% 左右）。除了这 15% 的翻牌，我们的权益始终很差：

![pics/4-1](pics/4-1.jpg)

与上面翻牌权益分布范围相反的例子，我们有优质、双同花的牌，例如 9♠8♥7♠6♥。这些牌通常会在翻牌击中不错的成手牌 + 听牌组合，让我们能够有利可图地跟注全押。从下面的翻牌权益分布图中，我们可以看到我们的权益现在分布均匀，而不是集中在少数前 x% 的翻牌上：

![pics/4-2](pics/4-2.jpg)

这意味着像这样 9♠8♥7♠6♥ 的牌经常击中翻牌，成为好牌足以在翻牌时获利。当我们跟注 4-bet 的对手预期翻牌 c-bet 时，我们获得的赔率略高于 2 : 1，因此我们需要在翻牌时获得略低于 33% 的权益。上图告诉我们，我们在各种翻牌上都有 33% 或更好的牌，大约占所有翻牌的前 60%。
...
CO（$10）加注到 $0.35，你（$10）持有 T♠8♦7♣6♠ 在 BTN 位置。你已经读到 CO 是一个尊重 3-bet 的松散加注者，并且在被 3-bet 后，他倾向于在不利位置上打的很紧。你在 BTN 有一手非常适合投机性 3-bet 的手牌，因此你 3-bet 底池到 $1.20。CO 立即 4-bet 底池到 $3.75。你的计划是什么？
...
CO（$6.25）全押。我们 T♠8♦7♣6♠ 在这个翻牌上有足够的权益（至少31%）吗？
