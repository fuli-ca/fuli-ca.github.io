...
因此，*在 3-bet 和 4-bet 底池中，我们根本不能 “坚果兜售”* （即耐心等待坚果或接近坚果的东西）。如果你在大底池中使用这种策略，你将被击败。大底池中正确的翻牌后打法通常要求你轻度地全下，有时只用顶对或非坚果对子 + 听牌组合。
...
翻牌后，在 SPR 极低的情况下，游戏通常归结为简单的底池赔率决策（跟注或弃牌），这对于 4-bet 单挑底池（当我们以 100 BB筹码开始）而言很常见。在 SPR 极低的情况下，我们必须愿意在单挑底池中很轻度地全下筹码。

在 SPR 较低的情况下，我们处于加注或弃牌区域。如果有人在我们面前下注，我们通常会选择弃牌或承诺完全跟注到底（通常会加注全下）。假设 100 BB 起始筹码和底池大小的加注，3-bet 单挑底池的 SPR 较低。因此，当我们的对手在翻牌时向我们下注时，我们将进行大量的加注或弃牌。
...
CO（$2.80）加注到 $0.35，你（$10）持有 A♣️A♥️9♣️8♠️ 在 BTN 位置 3-bet 底池到 $1.20，盲注弃牌，CO 跟注。

**Flop：**T♠️T♥️2♣️（$2.55）

CO（$1.60）将其余筹码推入。你的计划是什么？

这是跟注。原因是我们处于超低 SPR 场景中，我们有一个简单的底池赔率决定。翻牌时有效筹码为 $1.60，底池为 $2.55。我们有 SPR = 1.60 / 2.55 = 0.6，换句话说，SPR 超低，这是一个大底池场景，我们必须愿意轻度地进行全下。

CO 当然可能有三条，但他也可能很容易诈唬，或者用我们能击败的牌来争取价值。使用简单的逻辑，他的下注看起来就像他想让我们退出底池。在这种筹码量的情况下，他没有理由用一手强牌来全下。还剩下 3 条街可以玩，CO 可以随时下注。有了强牌，他应该简单地过牌给我们，并希望我们 c-bet。
...
你（$26）持有 A♣️A♥️9♣️8♠️ 在 BTN 位置加注到 $0.35，BB（$22.50）跟注。
...
如果翻牌时剩余的筹码为 1 个底池或更少，我们单挑可以用 A-A-x-x 在任何翻牌全下，而我们的对手无法剥削利用这一点（我们在第 3 部分进行了计算，证实了这一点）。就 SPR 而言，我们正在创建一个极低 SPR 场景，因为当翻牌圈后手的筹码少于 1 个底池大小时，我们的 SPR 小于 1。

当我们对短筹码进行 3-bet 时，同样的数学论证也有效，因为我们可以在翻牌前投入超过 1/3 的筹码。例如，如果一个拥有 30 BB 筹码的对手在 UTG 加注到 3.5 BB，而你在 BB 有一手垃圾 A-A-x-x 比如 A♠️A♦️9♣️6♦️，你可以 3-bet 底池到 11 BB，并投入超过 1/3 的筹码。当你被跟注时，你只需在翻牌圈将剩余的筹码推入即可。
...
BTN （$8.50）加注到 $0.35，你（$11.20）持有 A♣️Q♥️J♣️9♥️ 在 SB 位置 3-bet 到 $1.15，BTN 跟注。你知道 BTN 在有利位置会用各种牌加注。你还多次看到他单挑在翻牌有利位置加注 c-bet，而且他似乎在翻牌后用边缘牌 / 听牌在大底池中打的激进。

**Flop：**J♥️7♠️3♣️（$2.40）

你（$10.05）下注 $2.40，BTN（$7.35）加注全下。你的计划是什么？

让我们首先找到 SPR 值，然后评估我们手牌的强度，然后做出决定。翻牌时底池为 $2.40，后手筹码 $7.35，因此 SPR = 7.35 / 2.40 = 3.1。这是一个低 SPR，我们应该准备好在翻牌用各种成牌和听牌进行全下。因此，当我们用一手不错的牌对抗激进的对手时，应该以跟注全下为目标。
...
请注意，我们的顶对可以作为顶暗三条的阻挡牌，而且对手的范围似乎不太可能有很多 3-3-x-x。所以我们并不特别担心被暗三条击败。这手牌对手以很宽的范围开池，我们知道他也以很宽的范围在翻牌圈加注。所以他不需要碾压我们就可以在这里全下，而且我们领先的可能性也很大（例如，如果对手用占优势的顶对 + 听牌组合加注）。但如果我们落后，我们平均应该有 8-10 张补牌。这是一个粗略的估计，但似乎是合理的（从上面对 J-T-9-7 的补牌计数可以看出）。仅后门听牌就提供了 4 张相当干净的补牌，所以我们不需要太多其他东西就能得到我们想要的东西。

对手加注全下将底池增加到 $12.15，而跟注金额为 $4.95，因此我们的底池赔率为 12.15 : 4.95 = 2.45 : 1。因此，我们需要 1 / (2.45 + 1) = 0.29 = 29% 的权益才能跟注。如果我们总是落后，这相当于翻牌时大约有29 / 4 = 7.2 个补牌。

因此，即使我们总是落后，我们似乎也拥有所需的权益。考虑到对手的激进倾向，我们有时会领先，因此我们可以轻松跟注。

**Flop：**J♥️7♠️3♣️（$2.40）

你（$10.05）下注 $2.40，BTN（$7.35）加注全下，你跟注。

**Turn：**J♥️7♠️3♣️K♠️（$17.10）

**River：**J♥️7♠️3♣️K♠️7♦️（$17.10）
你以两对获胜。BTN 持有 Q♠️J♦️T♣️8♠️ 在翻牌时拥有顶对 + 卡顺听牌 + 后门同花听牌。

我们在翻牌前拿到一手优势牌，并以 60% 的翻牌前权益进行了 3-bet。翻牌圈双方都拿到了顶对，但我们的牌力更强，于是用权益是 62%的剩余筹码全下。

**示例 5.3.2：有利位置价值 3-bet 翻牌击中超对在非常协调的牌面上对抗扎实的玩家**

$10PLO 6-max

MP（$11.10）加注到 $0.35，你（$15.60）持有 A♥️A♠️6♥️5♠️ 在 BTN  位置 3-bet 底池到 $1.20，MP 跟注。
...
从技术上讲，通常在翻牌时我们的裸超对会是 “最好手牌”。但在奥马哈的翻牌中，“最好手牌” 一词通常没有什么意义。原因是听牌通常会在权益方面压倒当前最好的手牌。例如面对 A♣️K♦️Q♦️J♥️（顶对 + 包牌顺子听牌 + 后门同花听牌），我们的手牌在翻牌时技术上被 “击败”，我们有 45% 的权益。

我们最担心的是面对翻牌击中顺子时可能听死牌的风险，或者面对一手更好的成手牌 + 一手好听牌的组合时可能听到近乎死牌的风险。例如，我们在对抗 K♣️Q♦️9♦️8♣️（两对 + 同花听牌 + 卡顺听牌）时只有可怜的 14% 权益，并且我们发现在对手的范围内有很多像这样的牌。
...
请注意，价值 3-bet 和投机 3-bet 之间的界限是 “模糊的”。我们选择将价值 3-bet 定义为使用优质同花和协调高牌手牌进行的 3-bet。但（根据我们的定义）投机 3-bet 手牌（如 Q♥️J♠️T♥️9♠️）肯定也可以归类为优质牌。

如前所述，“价值 3-bet”、“投机 3-bet” 和 “诈唬 3-bet” 的这种分类主要是我们用来促进合理思维过程的概念工具。

对于优质高牌手牌（例如，优质 A-A-x-x），我们更经常独立于情景进行 3-bet。对于投机 3-bet 手牌（例如 A♣️8♣️7♠️5♠️ ），其高牌强度低于价值范围内的手牌，我们在 3-bet 之前往往会对情景更加挑剔。对于非常投机的诈唬 3-bet 牌（例如 K♦️Q♠️T♥️6♦️），我们总是要求在 3-bet 之前有有利条件（最有利位置和良好的偷牌权益）。
...
UTG（$10）加注到 $0.35。UTG 是一名扎实的玩家，但当他被 3-bet 时，他似乎打得非常直接。你持有 A♣️9♠️8♣️7♣️ 在 BTN 位置 3-bet 到 $1.20。盲注弃牌，UTG 跟注。
...
当我们把 3-bet 的范围扩大到包括像 Q♠️T♦️8♦️3♠️ 这样的半垃圾牌时，我们就进入了诈唬 3-bet 的领域。这并不比大幅放宽起手牌要求更复杂，目的是为了利用我们预期拥有良好偷盲权益的有利局面。
...
CO（$14.35）加注到 $0.35，你（$12.50）持有 K♦️J♦️8♣️3♣️ 在 BTN 位置。CO 加注范围很宽，他也用很宽的范围跟注 3-bet，但他在 3-bet 后打得紧而直白。盲注位置的玩家似乎很紧。你决定用一手非常投机的牌进行 3-bet 诈唬，并且 3-bet 底池到 $1.20。CO 跟注。
...
不。这在这里没有什么意义，因为没有任何转牌（除了 3♦️）可以让我们在转牌获得强牌或强听牌。因此，与示例 5.3.3 不同，过牌这个翻牌不会为我们带来许多有利可图的转牌场景，因为没有任何转牌可以让我们有信心继续下去。此外，当我们拥有这么少的权益时，被过牌 - 加注退出这个底池对我们来说不是问题。
...
- 我们在 BB 持有 A♠️A♥️J♠️T♥️（优质牌）或 A♠️A♥️7♦️2♣️（垃圾牌），对手知道我们的牌。
- 我们 3-bet 底池到 11 BB，计划 c-bet 所有翻牌并跟注全下加注。
- 对手用其整个范围跟注 3-bet。
- 我们 c-bet 所有翻牌，对手在有足够权益的所有翻牌上加注全下，否则他弃牌。

我们想要回答的问题是：

*对手能否利用其完全掌握的信息，在翻牌前加注，然后用包含前 50% 牌型（不包括所有 A-A-x-x 牌型）的宽范围跟注 3-bet，从而获利？*

请注意，我们已从他的范围中移除 A-A-x-x，以使事情更简单。当对手有 A-A-x-x 时，这手牌无论如何都会以翻牌前全下对抗结束，而且大部分底池都会被平分。此外，我们从对手将第一枚筹码放入底池的那一刻起计算他的 EV，因为他知道我们有 A-A-x-x，并且他会被 3-bet。然后，他跟注我们的 3-bet，假设我们会在所有翻牌上下注，并计划跟注加注。

翻牌时底池为 11 + 11 + 0.5 = 22.5 BB，而筹码还剩下 89 BB，SPR 为 89 / 22.5 = 4.0（因此我们处于加注或弃牌区域）。当对手在翻牌上加注时，他冒着 89 BB 的风险赢得 22.5 + 89 = 111.5 BB。因此，他获得的有效底池赔率为 112.5 : 89 = 1.25 : 1，并且他至少需要 1 / (1.25 + 1) = 0.44 = 44% 的权益才能有利可图地加注全下。

因此，对手将在他拥有 44% 或更多权益的 top_x% 翻牌上加注全下，否则他将弃牌并失去 11 BB 翻牌前的投资。当他加注全下时，他的平均权益是 av_equity，然后他将在 200.5 BB 的底池中全下，而他总共投资了 100 BB。

我们从翻牌权益分布图中找到了 top_x 和 av_equity。对手在 BTN 加注对抗我们已知的 A-A-x-x 牌的平均总 EV 是：

EV（总的）= (1 - top_x)(-11 BB) + top_x(av_equity(200.5 BB) - 100 BB)

以下是对手在面对我们的牌 A♠️A♥️J♠️T♥️ 和 A♠️A♥️7♦️2♣️ 时的前 50% 范围的翻牌权益分布图和计算。对于这两手牌，我们首先计算了对手加注时的总 EV。然后，我们还计算了对手跟注 3-bet 时的总 EV（对手在翻牌前冒 7.5 BB 的风险，总共冒 96.5 BB 的风险，因为 3.5 BB 的加注现在属于底池，而不是他）。
...
我们的模型首先告诉我们，对手不能通过参与对抗我们的优质 A♠️A♥️J♠️T♥️ 而获利（总体而言）。即使拥有完美的信息、完美的翻牌打法和最大的隐含赔率，当他选择加注对抗我们的优质 A-A-x-x 牌时，他也会损失 -1.32 BB。

但是当我们 3-bet 垃圾 A♠️A♥️7♦️2♣️ 时，对手实际上在加注时赚了 +7.54 BB，即使他知道我们有 A-A-x-x，即使他知道他每次都会被 3-bet！

我们还看到，对手跟注对抗 A♠️A♥️J♠️T♥️ 的 3-bet 是有利可图的，即使他总体上输掉了这手牌的筹码。他从翻牌前加注的那一刻起就输了，但是当他被 3-bet 时，他跟注并继续这手牌的损失比翻牌前弃牌的损失要小。
...
最后，我们观察到完美信息的价值是巨大的。对手在对抗我们时使用了非常广泛的前 50% 范围，但他仍然设法在对抗垃圾 A-A-x-x 时获得了可观的利润。前 50% 范围包括许多粗糙的牌，例如 Q♣️9♣️6♠️5♥️，但仍然有可能在拥有完美信息的情况下在对抗垃圾 A-A-x-x 时获利。此外，我们看到对手可以有利可图地跟注 3-bet，无论是对抗优质 A-A-x-x 还是垃圾 A-A-x-x，尽管他总体上输给了 A♠️A♥️J♠️T♥️（但他跟注 3-bet 的损失比弃牌的损失要少）。
...
细心的读者当然会发现，拥有明确 3-bet 范围的问题也可以通过 3-bet 非 A-A-x-x 牌来解决。但这并不能解决当我们拥有 A-A-x-x 和坏边牌时找不到足够好的翻牌来全下的问题。
...
带有坏边牌的对子（例如 K♠️K♥️7♦️2♣️）和投机性 A-x-x-x 牌（例如 A♠️T♥️9♠️2♦️）在 3-bet 底池中表现不佳。原因如下：
...
因此，当我们建立大底池时，我们更喜欢经常击中翻牌的同花和协调的牌。在大底池中表现不佳的牌是在争夺摊牌权益时更依赖隐含赔率的牌。K♠️K♥️7♦️2♣️ 或者 A♠️T♥️9♠️2♦️ 就是这种牌的例子。
...
为了说明这些原则（以防你们中的一些人仍然想用像 K♠️K♥️7♦️2♣️ 这样的牌跟注 3-bet 来击中三条），我们将对翻牌权益分布进行模型研究。我们假设我们加注干燥对子或投机性 A-x-x-x 并被 3-bet，然后我们跟注在翻牌上玩不中牌就弃牌。

我们的模型如下：

- 你用 K♠️K♥️7♦️2♣️ 或 A♠️T♥️9♠️2♦️ 在 CO 加注底池到 3.5 BB。
- BTN 用 A-A-x-x 3-bet 加注底池到 12 BB，盲注玩家弃牌。
- 如果我们知道 BTN 持有 A-A-x-x 并且他会在每次翻牌时下注 - 跟注，我们是否可以从隐含赔率中获利？

假设我们跟注 3-bet（再跟注 8.5 BB）。翻牌时底池现在为 25.5 BB。双方都以 100 BB 的筹码开始，还剩下 88 BB 可以玩。我们知道 BTN 持有 A-A-x-x，我们也知道他在翻牌后承诺完全投入。如果我们过牌，他会在所有翻牌时 c-bet，然后他总是跟注过牌 - 加注全下。

因此，我们的翻牌后计划是永远不要诈唬，而是在翻牌时过牌 -加注全下 BTN 玩家。我们冒着 88 BB 的风险赢得 25.5 BB + 对手的 88 BB = 113.5 BB。有效底池赔率为 113.5 : 88 = 1.29 : 2，因此我们需要 1 / (1.29 + 1) = 0.44 = 44% 翻牌圈的权益才能有利可图地全下。

因此，我们在 top_x% 上全下，其中我们的平均权益为 av_equity。然后我们在 201.5 BB 的底池中全下，从我们决定跟注 3-bet 的那一刻起，我们已经投入了 8.5 BB + 88 BB = 96.5 BB
...
以下是使用 K♠️K♥️7♦️2♣️ 和 A♠️T♥️9♠️2♦️ 对抗 A-A-x-x 跟注 3-bet 的翻牌权益分布和 EV 计算。请注意，我们在 ProPokerTools 计算中对手的手牌使用符号 AA** ! AAA*（这排除了对手可能不会 3-bet 的 A-A-A-x）：
...
我们观察到，在模型描述的条件下，K♠️K♥️7♦️2♣️（+2.58 BB）和 A♠️T♥️9♠️2♦️（+2.86 BB）都可以跟注 3-bet，获利不大。这是否意味着这些牌在被 3-bet 时适合跟注并在翻牌圈玩不中就弃牌？
...
例如，当你翻牌圈表现非常好并且他的权益很差时，有能力的对手有时会正确地下注 - 弃牌他的 A-A-x-x。如果他在翻牌圈下注底池（25.5 BB）并对你的加注全下弃牌，他将带着一半以上的筹码逃脱。这会降低你的隐含赔率（如果对手在某些翻牌圈下注 - 弃牌，他当然也会增加你的偷牌权益，但我们假设你在这里使用的是 “不中就弃牌” 策略）。其他时候，他会利用位置，在翻牌圈随后过牌，并以此方式节省筹码。
...
当你用对子带坏边牌（K♠️K♥️7♦️2♣️、J♠️T♦️T♠️4♣️ 等）或投机性 A-x-x-x （A♠️T♥️9♠️2♦️、A♦️K♦️9♠️5♥️ 等）加注时，无论是在有利位置还是不利位置，都应对 3-bet 弃牌。
...
如果对手牌技不佳，经常在翻牌后犯错，那么就应该跟他玩更多手牌（反之，面对水平较高的对手时，则应该弃掉更多牌）。一个典型的例子是，有些玩家在翻牌后过度玩一些较差的 A-A-x-x 牌，并且经常只用一个超对和很差的边牌就贸然全下。

当我们在翻牌前用垃圾 A-A-x-x 3-bet 时，这是模型研究的重要组成部分。用坏的 A-A-x-x 3-bet 本身就是一个错误，而我们通过在所有翻牌上承诺全下加剧了错误。这使得对手跟注我们的 3-bet 非常有利可图。
...
首先，即使对手的范围很紧，我们仍然会凭借一系列弱牌获得不错的权益。这是奥马哈，不是德州扑克，一手起手牌很少能有超过 60% 的翻牌前权益来对抗另一手牌。此外，PLO 起手牌最重要的属性不是全下翻牌前权益，而是翻牌后的权益分布（即翻牌后的权益分布图）。

例如，K♠️K♥️7♦️2♣️ 从技术上讲，在与 J♠️T♥️9♠️8♥️ 的对抗中，是 51% 翻牌前权益的热门。但 K♠️K♥️7♦️2♣️ 在 3-bet 底池中表现不佳，而 J♠️T♥️9♠️8♥️ 在单挑大底池场景中表现非常出色，因为它击中了很多翻牌。
...
你（$12.50）持有 J♠️7♠️6♥️4♥️ 在 BTN 位置加注底池到 $0.35，SB（$9.95）3-bet 底池到 $1.15，你跟注。SB 的 3-bet% 为 1.8，你之前曾看到他在单挑底池中用 A-A-x-x 在翻牌圈下注 - 跟注加注。

**Flop：**T♠️2♥️2♣️（$2.40）

SB（$8.85）过牌。你的计划是什么？

翻牌前的打法很简单。我们怀疑 SB 是只 3-bet A-A-x-x 的玩家群体中的一员，而且在不利位置时他可能持有优质 A-A-x-x。但我们有一手双同花投机牌，很少被对手的牌压制，而且我们会发现很多翻牌，我们可以承诺全下从中获利（如果我们愿意，我们可以使用翻牌权益分布数据来 “证明” 这一点）。
...
你（$10）持有 9♠️8♥️7♦️5♠️ 在 UTG 加注底池到 $0.35，CO（$10）3-bet 底池到 $1.20。他的 3-bet% 为 2.1%，而且他似乎在每次翻牌单挑时都 c-bet。我们的计划是什么？
...
请注意，即使我们翻牌击中了一手不错的牌，我们也经常会发现自己在翻牌后处于困境，原因很简单因为我们在不利位置。在有利位置的单挑中，我们通常有机会加注 c-bet 全下，但在不利位置的情况下，有能力的对手也会经常随后过牌。
...
我们绝对想在这个翻牌上投入我们的筹码，问题是如何实现这一点。在有利位置时，我们可以加注对手的 c-bet 全下或当对手过牌时下注。但在不利位置时，我们必须做出决定，我们必须在下注和过牌 - 加注之间做出选择。

幸运的是，我们有一个可以帮助我们的阅读。我们知道对手单挑时 c-bet 很多，所以最好是过牌给他，然后过牌 - 加注全下。

**Flop：**K♦️8♣️5♦️（$2.55）

你（$8.80）过牌，CO（$8.80）下注 $2，你过牌 - 加注底池到 $8.55，CO 弃牌。

结果不错，我们肯定不会对在翻牌赢得底池感到不高兴（底两对 + 卡顺听牌对跟注我们的牌来说并不是强牌）。如果对手下注 - 弃牌 A-A-x-x，他可能犯了一个错误，因为如果他全下剩余的筹码，他几乎有 2:1 的有效底池赔率。凭借略高于 33% 的权益，他应该跟注，我们看到随机 A-A-x-x 在这个翻牌有 35% 的权益。
...
例如，面对一个只用 A-A-x-x 3-bet、总是在翻牌单挑时 c-bet、下注 - 跟注加注过于频繁的对手，你可以用像 T♦️8♠️7♠️4♦️ 这样的牌跟注 3-bet，并在 J♠️T♠️6♥️ 翻牌上全下，而不会在对手跟注时感到恶心。你对抗随机 A-A-x-x 有 59% 的权益，并且你预计随机 A-A-x-x 会构成对手范围的大部分。
...
因此，与范围中只有 A-A-x-x 的差的对手相比，具有坚实且平衡的 3-bet 范围的有能力的对手在 c-bet 时会拥有更好的平均权益，在跟注你的全下加注时也会拥有更好的平均权益。这是因为：

- 有能力的对手对翻牌的 c-bet 更加挑剔，并且不会经常用较差的权益下注。
- 他的下注 - 跟注加注范围将包含更多好的投机牌，这些投机牌会压制你的弱投机牌，即使你的弱投机牌能很好地对抗他范围中的 A-A-x-x 部分。

因此，面对松散但坚实且平衡的 3-bet 范围，我们应该稍微收紧我们的跟注标准，以避免在翻牌后抛硬币时处于不利地位。当我们在单挑 3-bet 底池中全下时，我们将主要发现自己处于 55 / 45 类型的抛硬币场景中，如果幸运的话，则是 60 / 40。
...
你（$10）持有 K♦️K♣️Q♦️J♣️ 在 BTN 位置加注底池到 $0.35，BB（$9.90）3-bet 到 $1.10。对手是一个扎实、激进的玩家，在有利位置上有宽的 3-bet 范围，并且他还在单挑中用优质非 A-A-x-x 牌 3-bet。你的计划是什么？
...
你（$14.70）持有 K♦️J♦️8♠️7♥️ 在 CO 加注底池到 $0.35，BTN（$13.20）3-bet 底池到 $1.20。BTN 是一个扎实、激进的玩家，他 3-bet 范围很广（包括偶尔的诈唬 3-bet），但他的大多数 3-bet 都是优质高牌和好的投机牌。你的计划是什么？
