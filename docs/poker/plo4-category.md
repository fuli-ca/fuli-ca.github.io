...
A 在 PLO 中的价值非常大，如果一手牌没有 A，没有对子，没有好的连接性（连牌），好的同花组合，那么多数时候是很弱的牌。在 A 高牌里，通过观察，有两类牌权益差别较大，一类是 A 带 2 张高牌（K-T），另外就是其他的 A 高牌。这里细心的牌友会发现还有更好的 A 带 3 张高牌，这种组合被分配到 A 高连牌分类不包括在这里了。除了 A + 2 张高牌和 A + 其他边牌。这里还要区分 A 高双同花（包括 1 个 A 同花），A 高单同花（包括 1 个 A 同花），A 高没有 A 同花。高牌有人简称 H，这里用 B 代表百老汇牌。
...
关于 Rundown 的定义问题，Rundown 翻译成连牌，是手中一组连接牌和翻牌组成超过 8 张补牌的顺子听牌，这种结构在德州扑克里是没有的，德州扑克里最多是两头顺听牌有 8 张补牌。严格来说有 2 个缺口的连接牌也算连牌，比如 Q-J-T-7 可以和公共牌 9-8 组成 16 张补牌的顺子听牌。但是这种听牌要求公共牌刚好填补缺口，这种机会比较小，所以 7 的价值也就比较小，通常不如手里有一张 A。类似的 Q-J-9-7 也是有 2 个缺口的连牌，如果要发挥最大的威力，需要公共牌填补这两个缺口，同样机会也比较小。

讨论这个问题是因为在 GTO 软件里比如 PLO Mastermind 的 GTO 软件里，如果选择 Rundown 的组合条件，结果里是不包含 2 个缺口的连牌的，只有没有缺口的完美连牌和 1 个缺口的连牌。这种设置有几个原因，2 个缺口的连牌和完美连牌的权益差距很大，加上筛选条件里还有其他类似的连接牌的选项，比如 “近似百老汇连接牌”、“3 张低牌 + 1 张百老汇连接牌”、“没有百老汇牌的连接牌” 这些都是不严格的连接牌。

本文的连牌定义是包括 2 个缺口的连接牌，主要是不想把分类搞得太复杂太细，如果只包括软件里的 1 个缺口或者没有缺口的连牌，会有很多连接牌被排除在分类之外。另外本文是在 PLO5 分类文章之后改编的，在 PLO5 的分类里连牌用 2 个缺口的定义就比较合适。

在 PLO4 中评估连牌的权益的时候注意 2 个缺口的连牌和没有缺口的完美连牌相差很大，只有在某些情况比如在有位置优势的情况下才能适当放宽范围。
